...
<div class="animate-slide-up">
    ...
    <div class="flex items-center mb-6">
        <a href="{{ route('potreros.show', $potrero) }}" class="w-12 h-12 bg-white/20 hover:bg-white/30 text-white rounded-xl flex items-center justify-center mr-4 transition-all backdrop-blur-sm border-2 border-white/30">
            <i class="fa-solid fa-arrow-left text-xl"></i>
        </a>
        <h1 class="text-3xl font-bold text-white">Editar Potrero: {{ $potrero->nombre }}</h1>
    </div>

    <!-- Formulario -->
    <div class="glass-effect rounded-2xl shadow-xl p-8 border-4 border-white/50">
        <form action="{{ route('potreros.update', $potrero) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" value="{{ $potrero->codigo }}" readonly
                        class="w-full px-4 py-3 bg-gray-100 border-2 border-gray-300 rounded-xl text-gray-600 cursor-not-allowed">
                    <p class="text-gray-500 text-xs mt-1">El código no se puede modificar</p>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre', $potrero->nombre) }}" required
                    ...
                    <select name="finca_id" required
                        ...
                        @foreach($fincas as $finca)
                            <option value="{{ $finca->id }}" 
                                {{ old('finca_id', $potrero->finca_id) == $finca->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" name="area" value="{{ old('area', $potrero->area) }}" step="0.01" min="0"
                        placeholder="Ej: 15.5"
                        class="w-full px-4 py-3 bg-white border-2 border-gray-300 rounded-xl text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                    @error('area')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="tipo_pasto"
                        ...
                        @foreach($tipos_pasto as $key => $tipo)
                            <option value="{{ $key }}" 
                                {{ old('tipo_pasto', $potrero->tipo_pasto) == $key ? 'selected' : '' }}>
                                {{ $tipo }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="capacidad_animales" value="{{ old('capacidad_animales', $potrero->capacidad_animales) }}" min="0"
                        placeholder="Ej: 50"
                        class="w-full px-4 py-3 bg-white border-2 border-gray-300 rounded-xl text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                    <p class="text-gray-500 text-xs mt-1">Actualmente: {{ $potrero->animales_actuales }} animales</p>
                ...
                <div>
                    ...
                    <select name="estado" required
                        ...
                        @foreach($estados as $key => $estado)
                            <option value="{{ $key }}" 
                                {{ old('estado', $potrero->estado) == $key ? 'selected' : '' }}>
                                {{ $estado }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="dias_descanso" value="{{ old('dias_descanso', $potrero->dias_descanso) }}" min="0" max="365"
                        placeholder="Ej: 30"
                        class="w-full px-4 py-3 bg-white border-2 border-gray-300 rounded-xl text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                ...
                <div>
                    ...
                    <input type="date" name="fecha_ultima_rotacion" 
                        value="{{ old('fecha_ultima_rotacion', $potrero->fecha_ultima_rotacion ? $potrero->fecha_ultima_rotacion->format('Y-m-d') : '') }}" 
                        max="{{ date('Y-m-d') }}"
                        class="w-full px-4 py-3 bg-white border-2 border-gray-300 rounded-xl text-gray-800 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="observaciones" rows="4"
                        ...
                        class="w-full px-4 py-3 bg-white border-2 border-gray-300 rounded-xl text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">{{ old('observaciones', $potrero->observaciones) }}</textarea>
                    @error('observaciones')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex gap-4 mt-8">
                ...
                <a href="{{ route('potreros.show', $potrero) }}" class="flex-1 px-6 py-4 bg-gray-200 text-gray-700 hover:bg-gray-300 rounded-xl transition-all font-bold text-lg shadow-xl text-center">
